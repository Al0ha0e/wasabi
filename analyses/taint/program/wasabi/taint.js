...
// (function() {
    ...
    // export memory to the outside world
    // used in modified enc.js (emscripten wrapper) to track writes in JS to Wasm memory
    Wasabi.taintMemory = memory;
    // call after program completed to check if taint analysis was successful
    Wasabi.taintMemoryPrint = function () {
        return Wasabi.taintMemory.flatMap((value, index) => `${index}: ${value}`);
    }
    ...
    function Taint(label = 0) {
        this.label = label; // can hold any kind of more complex label; for now, just 0 (not tainted) and 1 (tainted)
    }
    ...
    Wasabi.analysis = {
        ...
        store(location, op, memarg, value) {
            ...
            // FIXME: only store taints that are actually non-zero
            if (taint.label !== 0) {
