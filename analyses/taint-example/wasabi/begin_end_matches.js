...
function block2str(type, {func, instr}) {
    return `${type} ${func}` + ((instr === -1) ? "" : `:${instr}`);
}
...
Wasabi.analysis = {
    ...
    begin(location, type) {
        blockStack.push(block2str(type, location));
        console.log(location, "begin", blockStack);
    },

    // ifLocation === location of the matching if block for else
    end(location, type, beginLocation, ifLocation) {
        let actual = block2str(type, beginLocation);
        let shadowed = blockStack.pop();
        if (actual !== shadowed)
            console.error(location, "actual:", actual, "shadowed:", shadowed);
    },
